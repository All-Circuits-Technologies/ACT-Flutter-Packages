...
typedef ViewDisplayCallback<T extends AbstractViewContext, C>
    = FutureOr<ViewDisplayResult<C>> Function(
  T viewContext,
...
abstract class AbstractViewBuilder {
  ...
  void registerViewDisplay<T extends AbstractViewContext>({
    ...
    if (_registeredBuilders.containsKey(context.uniqueKey)) {
      appLogger().e(
          "The view builder has already registered this context: $context, we don't "
          "override it");
      return;
    }

    _registeredBuilders[context.uniqueKey] =
        (context, result) async => callback(
              context as T,
              result,
            );
  }
  ...
  Future<ViewDisplayResult<C>> display<C>({
    ...
    if (builder == null) {
      if (_defaultDisplayCallback == null) {
        appLogger().e(
            "No view builder has been attached to this view context: $context and there is "
            "no default one, we can't proceed");
        return const ViewDisplayResult.error();
      }

      appLogger().w(
          "No view builder has been attached to this view context: $context, we use "
          "the default one");

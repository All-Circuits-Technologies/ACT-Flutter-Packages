...
class _QrCodeWidgetState extends State<QrCodeWidget> {
  ...
  Future<void> onQrControllerReady(
    ...
    controller.scannedDataStream.listen((Barcode scanData) async {
      ...
      if (widget.validator == null) {
        await onMatchedDataFound(context, code);
      } else {
  ...
  Future<void> onMatchedDataFound(
      BuildContext blocContext, String scanData) async {
    ...
    // In that case the bloc context will still be the same for getting the bloc
    // ignore: use_build_context_synchronously
    BlocProvider.of<QrCodeBloc>(blocContext)
        .add(const QrCodeFoundEvent(found: true));

    if (widget.playedOnSuccess != null) {
      await globalGetIt()
          .get<MusicPlayerManager>()
          .play(widget.playedOnSuccess!.value);
    }
  }
  ...
  Future<void> onWUnwantedDataFound(String scanData) async {
    ...
    if (scanData != _lastUnwantedScannedData) {
      ...
      if (widget.playedOnError != null) {
        await globalGetIt()
            .get<MusicPlayerManager>()
            .play(widget.playedOnError!.value);
      }

...
abstract class HaloPacketUtility {
  ...
  static List<Uint8List> formatPackets({
    ...
    for (var idxPacket = 0; idxPacket < packetNb; ++idxPacket) {
      ...
      packets.add(
          Uint8List.fromList(temporary.sublist(idxPacket * packetSize, end)));
    }
  ...
  static List<PayloadPacketElem>? extractFromPayloadPacket(
      Uint8List payloadPacket) {
    if (payloadPacket.isEmpty) {
      appLogger()
          .w("The HALO payload packet is empty, nothing can't be extracted");
      return null;
    }
    ...
    if (payloadPacket[0] != HaloPacketUtility._startByte ||
      ...
      appLogger().w(
          "The HALO payload packet received isn't complete, we haven't the start and/or "
          "end byte");
      return null;
    }
    ...
    elements.add(Uint8List.fromList(
        payloadPacket.sublist(elemIdx, (payloadLength - 1))));
  ...
  static (String, DateTime?)? getString(PayloadPacketElem element) {
    ...
    if (tmp == null) {
      appLogger().w(
          "A problem occurred when tried to get the string representation from a payload "
          "packet element");
      return null;
    }
  ...
  static PayloadPacketElem formatBoolean(
    ...
    tmp[0] =
        toSend ? HaloPacketUtility._trueValue : HaloPacketUtility._falseValue;
  ...
  static (bool, DateTime?)? getBoolean(PayloadPacketElem element) {
    ...
    if (tmp == null) {
      appLogger().w(
          "A problem occurred when tried to get the boolean representation from a "
    ...
    if (data.isEmpty) {
      appLogger()
          .w("We can't get a boolean from an empty payload packet element");
      return null;
    }
  ...
  static PayloadPacketElem? formatUInt8(
    ...
    DateTime? ts,
      ...
      HaloPacketUtility._formatNumber(number, ByteUtility.bytesNbUInt8, false,
          ts: ts);
  ...
  static PayloadPacketElem? formatUInt16(
    ...
    DateTime? ts,
      ...
      HaloPacketUtility._formatNumber(number, ByteUtility.bytesNbUInt16, false,
          ts: ts);
  ...
  static PayloadPacketElem? formatUInt32(
    ...
    DateTime? ts,
      ...
      HaloPacketUtility._formatNumber(number, ByteUtility.bytesNbUInt32, false,
          ts: ts);
  ...
  static PayloadPacketElem? formatInt8(
    ...
    DateTime? ts,
      ...
      HaloPacketUtility._formatNumber(number, ByteUtility.bytesNbUInt8, true,
          ts: ts);
  ...
  static PayloadPacketElem? formatInt16(
    ...
    DateTime? ts,
      ...
      HaloPacketUtility._formatNumber(number, ByteUtility.bytesNbUInt16, true,
          ts: ts);
  ...
  static PayloadPacketElem? formatInt32(
    ...
    DateTime? ts,
      ...
      HaloPacketUtility._formatNumber(number, ByteUtility.bytesNbUInt32, true,
          ts: ts);
  ...
  static PayloadPacketElem? formatInt64(
    ...
    DateTime? ts,
      ...
      HaloPacketUtility._formatNumber(number, ByteUtility.bytesNbUInt64, true,
          ts: ts);
  ...
  static PayloadPacketElem? _formatNumber(
    ...
    if (tmp == null) {
      appLogger().w(
          "A problem occurred when tried to convert the number: $number to LSB, with "
  ...
  static (int, DateTime?)? getNumber(PayloadPacketElem element, bool isSigned) {
    ...
    if (tmp == null) {
      appLogger().w(
          "A problem occurred when tried to get the number representation from a "
          "payload packet element");
      return null;
    }
    ...
    final value =
        ByteUtility.convertFromLsb(lsbNumber: data, isSigned: isSigned);

    if (value == null) {
      appLogger().w(
          "A problem occurred when tried to convert the byte list to a number");
      return null;
    }
  ...
  static PayloadPacketElem _formatAndEscapePacketElem(
      Uint8List element, DateTime? timestamp) {
    ...
    for (var idx = 0; idx < tmp.length; ++idx) {
      ...
      if (value == HaloPacketUtility._escapeElement) {
        if (idx == (tmp.length - 1)) {
          ...
          appLogger().w(
              "There is an escape character at the end of a payload element, this case "
  ...
  static (Uint8List, DateTime?)? _extractDataFromPacketElem(
    ...
    if (tsSepIndex >= 0) {
      ...
      final tmpTs =
          _unescapeGivenElementIfNeeded(element.sublist(0, tsSepIndex));
      if (tmpTs == null) {
        appLogger().w(
            "A problem occurred when tried to unescape a timestamp from a payload packet "
            "element");
        return null;
      }

      final tsInMS =
          ByteUtility.unsafeConvertFromLsb(lsbNumber: tmpTs, isSigned: false);
    ...
    if (unTmpElem == null) {
      appLogger().w(
          "A problem occurred when tried to unescape a value from a payload packet "
          "element");
      return null;
    }
  ...
  static bool? isLastElementPacket(Uint8List packet) {
    ...
    if (packet.contains(HaloPacketUtility._endByte)) {
      appLogger().w(
          "The end byte has been seen in the packet received but this should never "

...
/// in the device.
abstract class AbstractHaloRequestToDeviceMaterial extends AbstractHaloComponentMaterial {
  /// This method allows to call a function in the device, a function always returns a value
  Future<HaloRequestResult> callFunction({required HaloRequestParamsPacket request}) async {
    final error = _verifyRequest(request: request, expectedType: HaloRequestType.function);
    if (error != null) {
      return error;
    }

    final result = await implCallFunction(request: request);
  ...
  @protected
  Future<HaloRequestResult> implCallFunction({required HaloRequestParamsPacket request});
  ...
  Future<HaloErrorType> callProcedure({required HaloRequestParamsPacket request}) async {
    final error = _verifyRequest(request: request, expectedType: HaloRequestType.procedure);
    if (error != null) {
      return error.error;
    }

    return implCallProcedure(request: request);
  }
  ...
  @protected
  Future<HaloErrorType> implCallProcedure({required HaloRequestParamsPacket request});

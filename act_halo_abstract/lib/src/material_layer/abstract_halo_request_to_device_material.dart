...
/// in the device.
abstract class AbstractHaloRequestToDeviceMaterial
  ...
  Future<HaloRequestResult> callFunction(
    ...
    final error = _verifyRequest(
        request: request, expectedType: HaloRequestType.function);
    if (error != null) {
      return error;
    }
    ...
    if (result.error == HaloErrorType.noError && result.result == null) {
      appLogger().w(
          "The function: ${request.requestId} succeeds but no result has been returned");
  ...
  Future<HaloErrorType> callProcedure(
      {required HaloRequestParamsPacket request}) async {
    final error = _verifyRequest(
        request: request, expectedType: HaloRequestType.procedure);
    if (error != null) {
      return error.error;
    }
  ...
  HaloRequestResult? _verifyRequest({
    ...
    if (request.requestId.type != expectedType) {
      appLogger()
          .w("A problem occurred in the HALO lib, you tried to call a request: "

...
import 'package:act_thingsboard_client/src/mixins/mixin_thingsboard_env.dart';
...
class TbRequestService<E extends MixinThingsboardEnv,
    S extends MixinThingsboardSecret> extends AbstractService {
  ...
  TbRequestService({
    ...
    unawaited(_authInitLock
        .waitAndLock()
        .then((value) => _authInitLockEntity = value));
  }
  ...
  Future<void> initService() async {
    ...
    final uri =
        Uri(port: port, host: hostname, scheme: ServerReqConstants.httpsScheme);
    ...
    Duration? retryTimeout,
      ...
      _initAndSignInMutexProtect(() async {
        ...
        if (loginResponse.result != RequestResult.success) {
          _logsHelper.w(
              "A problem occurred when tried to sign in the user thanks to the identifiers "
              "given");
          return false;
        }
  ...
  Future<TbRequestResponse<T>> _safeRequestImpl<T>(
    ...
    while (globalResult != RequestResult.success &&
      ...
      retryRequestNb++;
      (globalResult, tbResult) =
          (await _safeRequestNoAuth(requestToCall)).toPatterns();
      ...
      if (useAuth &&
        ...
        if (!(await _signInFromMemoryImpl())) {
          _logsHelper.e(
              "There is a problem when tried to log in, may be the logins aren't right?");
  ...
  Future<TbRequestResponse<T>> _safeRequestNoAuth<T>(
      TbRequestToCall<T> requestToCall) async {
  ...
  Future<bool> _signInFromMemoryImpl() => _signInMutex.protect(() async {
        ...
        if (username == null || password == null) {
          _logsHelper.i(
              "Can't sign user from memory: the identifiers aren't stored in app");
          return false;
        }

        final loginResponse = await _safeRequestImpl(
          (tbClient) async =>
              tbClient.login(LoginRequest(username!, password!)),
          useAuth: false,
        );
  ...
  Future<void> _initAuthUserAtStart() async {
    ...
    if (result.result == RequestResult.success &&
        _tbClient.getAuthUser() == null) {
      _logsHelper.d(
          "There is no thingsboard tokens stored in app, we try to sign from memory");
  ...
  /// the given method
  Future<T> _initAndSignInMutexProtect<T>(
      Future<T> Function() criticalSection) async {

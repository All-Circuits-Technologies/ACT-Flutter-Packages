...
class FixedExtentListScrollPhysics extends ScrollPhysics {
  ...
  Simulation? createBallisticSimulation(
    ...
    final tmpTolerance = toleranceFor(position);

    // Scenario 3:
    // If there's no velocity and we're already at where we intend to land,
    // do nothing.
    if (velocity.abs() < tmpTolerance.velocity &&
        (settlingPixels - position.pixels).abs() < tmpTolerance.distance) {
      return null;
    }
    ...
    return FrictionSimulation.through(
      ...
      settlingPixels,
      velocity,
      tmpTolerance.velocity * velocity.sign,
    );
  }
}

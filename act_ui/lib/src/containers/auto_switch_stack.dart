...
class AutoSwitchStack extends StatefulWidget {
  ...
  AutoSwitchStack({
    ...
    required this.stack,
            ...
            "Because we want to create an animation the duration can't be equal to zero"),
...
class _AutoSwitchStackState extends State<AutoSwitchStack> {
  ...
  late ValueStream<int> _ticStream;
  ...
  void _refresh() {
    final tic = _ticStream.value;

    final wantedIndex = widget.stack.children.isEmpty
        ? null
  ...
  @override
  void dispose() {
    unawaited(_ticSubscription?.cancel());
    super.dispose();
  }
  ...
  Widget build(BuildContext context) => IndexedStack(
        ...
        children: (_currentIndex == null)
            ? <Widget>[const SizedBox()] + widget.stack.children

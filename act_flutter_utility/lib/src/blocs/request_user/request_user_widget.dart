...
typedef RequestUiBlocBuilder<B extends RequestUserUiBloc> = B Function(
    BuildContext context);

/// Display an UI used to request the user for a specific action
abstract class AbstractRequestUserUiWidget<B extends RequestUserUiBloc>
  ...
  Widget build(BuildContext context) => BlocProvider<B>(
        ...
        child: BlocBuilder<B, RequestUserUiState>(
          builder: (context, state) {
            final refusalButton =
                _buildRefusalButton(context: context, state: state);

            return PopScope(
              canPop: false,
              onPopInvoked: (_) {
                if (refusalButtonBuilder != null || state.isOk) {
              ...
              child: _wrapScaffold(
                ...
                child: Column(
                  children: [
                      ...
                      child: SingleChildScrollView(
                        child: Column(
                          children: childrenBuilder(
                              context, state.isOk, state.loading),
                        ),
                      ),
                    ),
                    ...
                    if (spaceAfterButtons != null)
                      SizedBox(height: spaceAfterButtons),
                  ],
                ),
              ),
            );

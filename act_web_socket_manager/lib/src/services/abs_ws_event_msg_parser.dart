...
/// This is the abstract class to parse the received message by the web socket
abstract class AbsWsEventMsgParser<Event extends MixinStringValueType> extends AbsWithLifeCycle
  ...
  /// {@macro act_web_socket_manager.MixinWsMsgParserService.onMessageReceived}
  @override
  ...
  Future<void> onMessageReceived(dynamic message) async {
    ...
    if (message is! String && message is! Map<String, dynamic>) {
      _logsHelper.w(
        "The message received by the web socket isn't a string or JSON object; therefore we can't "
        "parse it",
      );
      return;
    }
    ...
    if (jsonMsg == null || jsonMsg is! Map<String, dynamic>) {
      _logsHelper.w(
        "The message received by the web socket isn't a JSON object; therefore we can't parse "
        "it",
      );
      return;
    }
    ...
    if (event == null) {
      _logsHelper.w(
        "The message received by the web socket doesn't contain an event field, with key: "
    ...
    if (!jsonMsg.containsKey(dataJsonKey)) {
      _logsHelper.w(
        "The message received by the web socket doesn't contain a data field, with key: "

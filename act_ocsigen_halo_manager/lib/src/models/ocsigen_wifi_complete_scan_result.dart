...
class OcsigenWiFiCompleteScanResult extends Equatable {
  ...
  static List<OcsigenWiFiCompleteScanResult>? parseFromDevice(
      HaloPayloadPacket packet) {
    final length = packet.elementsNb;

    if ((length % elementsNb) != 0) {
      appLogger().w(
          "The list of WiFi complete info isn't well formatted, the number of elements "
    ...
    for (var idx = 0; idx < length; idx = idx + elementsNb) {
      ...
      if (rssiResult == null) {
        appLogger().w(
            "The element at the idx: ${idx + rssiIdx} isn't an int rssi, we can't "
            "parse the Ocsigen WiFi complete scan result");
        return null;
      }
      ...
      if (channelNbResult == null) {
        appLogger().w(
            "The element at the idx: ${idx + channelNbIdx} isn't an uint channel nb, we "
      ...
      if (tmpAuthMode == null) {
        appLogger().w(
            "The element at the idx: ${idx + authModeIdx} isn't an uint auth mode, we "
            "can't parse the Ocsigen WiFi complete scan result");
        return null;
      }
      ...
      if (authMode == OcsigenWiFiAuthMode.unknown) {
        appLogger().w(
            "The ocsigen WiFi auth mode isn't known: ${tmpAuthMode.$1}, we continue");
      }

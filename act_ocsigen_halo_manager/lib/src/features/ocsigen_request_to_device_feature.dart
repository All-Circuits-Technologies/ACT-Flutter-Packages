...
class OcsigenRequestToDeviceFeature<MaterialType>
    extends HaloRequestToDeviceFeature<MaterialType> {
  /// Class constructor
  ...
  Future<List<String>?> wiFiSsidScan({
    ...
    if (result.error != HaloErrorType.noError) {
      appLogger().w(
          "A problem occurred when calling the wifi ssid scan function, can't proceed");
      return null;
    }
  ...
  Future<List<OcsigenWiFiCompleteScanResult>?> wiFiCompleteScan({
    ...
    if (result.error != HaloErrorType.noError) {
      appLogger().w(
          "A problem occurred when calling the wifi complete scan function, can't "
          "proceed");
      return null;
    }
  ...
  Future<OcsigenWiFiConnectResult?> wiFiConnect({
    ...
    final result =
        await callFunction(materialType: materialType, request: packet);

    if (result.error != HaloErrorType.noError) {
      appLogger().w(
          "A problem occurred when calling the wifi connect function, can't proceed");
      return null;
    }
  ...
  Future<List<OcsigenWiFiStatusResult>?> wiFiGetStatus({
    ...
    if (result.error != HaloErrorType.noError) {
      appLogger().w(
          "A problem occurred when calling the wifi get status function, can't "
          "proceed");
      return null;
    }
  ...
  Future<int?> echo({
    ...
    if (!payload.addUInt8(uInt8ToRepeat)) {
      appLogger().w(
          "The parameter given is not an uint8, we can't call the echo function");
      return null;
    }
  ...
  Future<bool?> setGpsCoordinates({
    ...
    if (!payload.addUInt8(decimalPoint)) {
      appLogger()
          .w("We can't add the decimal point value to the payload packet");
      return null;
    }
  ...
  HaloRequestId? _getRequestId(OcsigenRequestId id) {
    ...
    if (requestId == null) {
      appLogger().w(
          "The request id: $id isn't known by the HALO manager, can't proceed");
      return null;
    }

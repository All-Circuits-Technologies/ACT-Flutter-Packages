...
class BleGattCharacteristicService extends AbstractService {
  ...
  Future<CharacteristicsError> writeBleCharacteristic(
    ...
    bool withoutResponse = false,
      ...
      _bleMutex.protect(() async {
        final (result, tmpChar) =
            await _verifyDeviceAndGetWantedChar(device: device, uuid: uuid);
        ...
        try {
          ...
          if (withoutResponse && !globalGetIt().get<PlatformManager>().isIos) {
            await _flutterBle.writeCharacteristicWithoutResponse(characteristic,
                value: values);
          } else {
            await _flutterBle.writeCharacteristicWithResponse(characteristic,
                value: values);
          }
  ...
  /// null
  Future<(CharacteristicsError, QualifiedCharacteristic?)>
      _verifyDeviceAndGetWantedChar({
    required BleDevice device,
    required String uuid,
  }) async {
    if (device.connectionState != DeviceConnectionState.connected) {
      _bleManager.logsHelper
          .w("The device you want to interact with, isn't connected: "
              "${device.id}");
    ...
    if (!(await _bleManager.checkAndAskForPermissionsAndServices())) {
      ...
      _bleManager.logsHelper.w(
          "The user don't have accepted the BLE permission or started the BLE "
    ...
    if (characteristic == null) {
      _bleManager.logsHelper
          .w("The characteristic: $uuid, hasn't been found in the "
  ...
  Future<CharacteristicsError> _manageDeviceInteractionError({
    ...
    if (_isIosBondingError(error)) {
      ...
      device.bondedState = BondState.bondingFailed;
  ...
  Future<void> _onSubscribeErrors(Object error) async {
    _bleManager.logsHelper
        .e("A non managed error occurred in the subscribe channel $error");
    ...
    if (errString.contains(error_messages.gattSuccessDisconnectedError)) {
      _bleManager.logsHelper
          .e("A disconnection has been detected, we disconnect the last "

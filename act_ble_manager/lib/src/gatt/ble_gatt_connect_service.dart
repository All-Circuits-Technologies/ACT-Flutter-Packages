...
class BleGattConnectService extends AbstractService {
      ...
      _bleMutex.protect(() async {
        if (!(await _bleManager.checkAndAskForPermissionsAndServices())) {
          ...
          _bleManager.logsHelper
              .w("The user don't have accepted the BLE permission or Ble isn't "
        ...
        device.bondedState = BondState.unknown;
        ...
        _deviceConnectionSub = _lastConnectedDevice!.connectionStateStream
            .listen(_onDeviceConnectionChanged);
        ...
        do {
          try {
            ...
            device.bondedState = BondState.bonded;
          } catch (error) {
            var bondedState = BondState.bonded;

            _bleManager.logsHelper
                .w('An error occurred when tried to discover the services of '
            ...
            device.bondedState = bondedState;
          }

          if (device.bondedState == BondState.bonding) {
            // We wait some times before retesting
            await Future.delayed(const Duration(seconds: 2));
          }
        } while (device.bondedState == BondState.bonding);
  ...
  Future<bool> _manageLowLevelConnection(BleDevice device) async {
    ...
    while (device.connectionState != DeviceConnectionState.connected &&
        !leftDuration.isNegative) {
      if (!_bleManager.hasPermissions || !_bleManager.isEnabled) {
        _bleManager.logsHelper
            .w("Can't proceed, we haven't the right to connect with BLE");
        return false;
      }
      ...
      final connSub = device.connectionStateStream
          .listen((DeviceConnectionState connState) {
        if (connState == DeviceConnectionState.connecting) {
          ...
          atLeastOneConnecting = true;
            ...
            (connState == DeviceConnectionState.disconnected &&
                atLeastOneConnecting)) {
          _bleManager.logsHelper
              .d('Can complete: $connState, at least one connecting: '
                  '$atLeastOneConnecting');
          completer.complete();
        }
      });
      ...
      try {
        ...
        _bleManager.logsHelper
            .w('The connection of the device: ${device.id}, failed: $e');
      }
      ...
      await completer.future.timeout(ble_constants.lowLevelConnectTimeout,
          onTimeout: () {
      ...
      if (device.connectionState != DeviceConnectionState.connected) {
        ...
        _bleManager.logsHelper
            .d("If the device isn't connected, we force the disconnection");
  ...
  Future<void> _tryToCleanGattCache({required BleDevice device}) async {
    ...
    try {
      ...
      if (error
          .toString()
          .contains(error_messages.clearCacheDeviceNotConnected)) {
        ...
        _bleManager.logsHelper.e(
            "A problem occurred when tried to clear the gatt cache linked to "
  ...
  Future<void> _onDeviceConnectionChanged(
      DeviceConnectionState deviceState) async {

...
class BleGapService extends AbstractService {
  ...
  List<Uuid> _deviceAdvServiceUuidToSearch;

  /// Getter of service advertising UUIDs for filtering devices
  List<Uuid> get deviceAdvServiceUuidToSearch => _deviceAdvServiceUuidToSearch;
  ...
  Future<void> setDeviceAdvServiceUuidsToSearch(List<Uuid> uuids) async {
    if (listEquals(_deviceAdvServiceUuidToSearch, uuids)) {
      return;
    }
  ...
  Future<bool> _launchScan() async {
    ...
    try {
      // TODO(brolandeau): Normally, to gain time when scanning, we have to give some services of the device to filter them, but this isn't working for now
      _scanSub = _flutterBle
          .scanForDevices(
            withServices: _deviceAdvServiceUuidToSearch,
            scanMode: _currentScanMode!,
          )
          .timeout(
  ...
  Future<void> _addScanDeviceCallback(DiscoveredDevice discoveredDevice) async =>
      _bleAccessMutex.protect(() async {
        ...
        if (keyFound != null) {
          ...
          final scannedDevice = _deviceMap[keyFound] as BleScannedDevice;

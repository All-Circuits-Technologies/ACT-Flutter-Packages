...
sealed class BodyFormatUtility {
  ...
  static BaseRequest? formatRequest({
    ...
    final convertedBody =
        _convertRequestBody(body: requestParam.body, logsHelper: logsHelper);
    ...
    if (convertedBody.contentType != MimeTypes.empty &&
      ...
      logsHelper.d(
          "We had guessed the content type: ${convertedBody.contentType.str}, and we set "
          "it in the request header");
      request.headers[ServerReqConstants.contentTypeHeader] =
          convertedBody.contentType.str;
    }
  ...
  static RequestResponse<Body> formatResponse<Body>({
    ...
    if (responseReceived.statusCode < HttpStatus.ok ||
      ...
      logsHelper.t(
          "The response received isn't ok, http status: ${responseReceived.statusCode}, "
    ...
    if (!result) {
      logsHelper.w(
          "An error occurred when tried to parse body from response of request: "
          "$urlToRequest");
  ...
  static (bool, Body?) _parseResponseBody<Body>({
    ...
    if (!responseReceived.headers
            .containsKey(ServerReqConstants.contentTypeHeader) &&
        !responseReceived.headers
            .containsKey(ServerReqConstants.contentTypeHeader.toLowerCase())) {
    ...
    try {
      ...
      logsHelper.w(
          "An error occurred when tried to parse the body of a response received, from "

...
class HaloRequestToDeviceFeature<MaterialType>
    extends AbstractHaloFeature<MaterialType> {
  /// Class constructor
  ...
  Future<HaloRequestResult> callFunction({
    ...
    required HaloRequestParamsPacket request,
      ...
      _callAndRetryResult(
        ...
        callRequest: (service) =>
            service.requestToDeviceMaterial.callFunction(request: request),
      );
  ...
  Future<T?> _callOneReturnFunction<T>({
    ...
    final result =
        await callFunction(materialType: materialType, request: request);

    if (result.error != HaloErrorType.noError) {
      appLogger().w(
          "A problem occurred when calling the ${request.requestId} function, can't "
          "proceed");
      return null;
    }
    ...
    if (resultPacket.elementsNb != 1) {
      appLogger().w(
          "The ${request.requestId} call hasn't returned the expected return");
      return null;
    }
  ...
  Future<HaloErrorType> callProcedure({
    ...
    required HaloRequestParamsPacket request,
      ...
      _callAndRetryError(
        ...
        materialType: materialType,
        callRequest: (service) =>
            service.requestToDeviceMaterial.callProcedure(request: request),
      );
  ...
  Future<HaloErrorType> callOrder({
    ...
    required HaloRequestParamsPacket request,
      ...
      _callAndRetryError(
        ...
        materialType: materialType,
        callRequest: (service) =>
            service.requestToDeviceMaterial.callOrder(request: request),
      );
  ...
  Future<HaloRequestResult> _callAndRetryResult({
    required MaterialType materialType,
    required HaloRequestId requestId,
    required Future<HaloRequestResult> Function(AbstractHaloMaterial)
        callRequest,
  }) =>
      _callAndRetry(
        materialType: materialType,
  ...
  Future<T> _callAndRetry<T>({
    ...
    final service = haloManagerConfig
        .materialLayer.materialServices[materialType]?.haloMaterial;

    if (service == null) {
      appLogger().w(
          "The material layer is unknown, we can't call request to the device");
    ...
    while (isMakingSensToRetry(result) &&
        nbTry < haloManagerConfig.retryNbBeforeReturningError) {

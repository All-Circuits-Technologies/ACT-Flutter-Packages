...
sealed class PermissionElementHelper {
  ...
  /// Return the library [permission_handler.Permission] attached to the
  /// [PermissionElement]
  static List<permission_handler.Permission> getPermissions(
    ...
    switch (element) {
      ...
      case PermissionElement.ble:
        if (isAndroid) {
          // Is Android
          if ((version ?? androidBleWithLocationMaxVersion) <
              androidBleWithLocationMaxVersion) {
            // Older Android SDK
            permissions.add(permission_handler.Permission.locationWhenInUse);
      ...
      case PermissionElement.background:
        if (isAndroid) {
          permissions
              .add(permission_handler.Permission.ignoreBatteryOptimizations);
        }
        break;
      ...
      case PermissionElement.trackingAuthorization:
        if (isIos) {
          permissions
              .add(permission_handler.Permission.appTrackingTransparency);
        }
        break;
    }

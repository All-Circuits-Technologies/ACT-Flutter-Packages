...
class HaloBleRequestToDeviceCmdPacket extends Equatable
    with InterfaceHaloPacketToSend {
  /// The command id
  ...
  const HaloBleRequestToDeviceCmdPacket({required this.packetToSend})
      : cmdId = HaloCmdId.call;

  /// Private constructor
  const HaloBleRequestToDeviceCmdPacket._(
      {required this.cmdId, required this.packetToSend});

  /// Helpful method to create a reset command
  factory HaloBleRequestToDeviceCmdPacket.reset(
  ...
  factory HaloBleRequestToDeviceCmdPacket.readReady(
          {required HaloRequestId requestId}) =>
  ...
  Uint8List getDataToSend() {
    ...
    // Add the CMD_ID
    toSend.add(cmdId.hexValue);
    // Add the REQUEST_ID : U1
    toSend.add(packetToSend.requestId.id);
    // Add the REQUEST_TYPE : EU1
    toSend.add(packetToSend.requestId.type.hexValue);
    // Add the NB_PARAM : U1

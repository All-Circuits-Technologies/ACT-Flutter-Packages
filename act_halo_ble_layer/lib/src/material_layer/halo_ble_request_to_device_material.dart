...
class HaloBleRequestToDeviceMaterial extends AbstractHaloRequestToDeviceMaterial {
  ...
  Future<HaloRequestResult> implCallFunction({required HaloRequestParamsPacket request}) =>
      _callRequest(cmdPacket: HaloBleRequestToDeviceCmdPacket(packetToSend: request));
  ...
  @override
  Future<HaloErrorType> implCallProcedure({required HaloRequestParamsPacket request}) async {
    final result = await _callRequest(
        cmdPacket: HaloBleRequestToDeviceCmdPacket(
      packetToSend: request,
    ));

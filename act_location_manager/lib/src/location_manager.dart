...
class DerivedLocationBuilder<T extends LocationManager> extends ManagerBuilder<T>
...
class LocationManager extends AbstractManager with MPermissionsService, MEnableService {
  ...
  /// If false, no HMI displayed is and we redirect the user to the system permissions page or
  /// activation page.
  ///
  ...
  Future<void> onLocationError(Object error) async {
    appLogger().e("An error occurred in the location library: $error");
  }
  ...
  Future<Position?> getCurrentPosition({
    ...
    if (!(await checkAndAskForPermissionsAndServices(
      ...
      appLogger().w("We can't get the current location: the permissions and services aren't "
          "activated");
      return null;
    }
    ...
    try {
      position = await Geolocator.getCurrentPosition(
        ...
        desiredAccuracy: overrideDefaultAccuracy ?? _initConfig.accuracy,
      );
    } catch (error) {
      appLogger().e('We failed to get the current position: $error');
  ...
  Future<Position?> getLastKnownPosition({
    ...
    if (!(await checkAndAskForPermissionsAndServices(
      ...
      appLogger().w("We can't get the last known location: the permissions and services aren't "
          "activated");
      return null;
    }
    ...
    try {
      ...
      appLogger().e('We failed to get the last known position: $error');
      if (error is LocationServiceDisabledException) {
        setEnabled(false);
      }
    }

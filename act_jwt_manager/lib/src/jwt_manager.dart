...
class JwtManager extends AbstractManager {
  ...
  Future<bool> addAndInitJwtHandler(AbstractJwtHandler handler) async {
    if (!(await handler.initHandler())) {
      logsHelper.w(
          "A problem occurred when tried to initialize the JWT handler given: "
          "${handler.name}");
      return false;
    }
    ...
    if (handler.canSignAndVerify && (!(await handler.testSignAndVerify()))) {
      logsHelper.w(
          "A problem occurred when tried to test the JWT handler given: ${handler.name}");
      return false;
    }
